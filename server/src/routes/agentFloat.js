...
router.post('/session/open', checkPermission('manage_agent_float'), async (req, res) => {
  try {
    ...
    dispatchToSupervisors(
      ...
      { business_id: bid(req), sender_user_id: req.user.id }
    );
...
router.post('/transactions', checkPermission('record_agent_float'), async (req, res) => {
  try {
    ...
    dispatchToSupervisors(
      ...
      {
        actor_name: req.user.name,
        cashier_name: transaction.cashier_name || session.cashier_name,
  ...
  try {
    ...
    dispatchToSupervisors(
      'CASH_FLOAT_CLOSED',
      {
        ...
        float_variance: result.float_variance,
      },
      { business_id: bid(req), sender_user_id: req.user.id }
    );
    ...
    const existing = await db
      .prepare(
        `SELECT t.*, u.name as cashier_name
         ...
         WHERE t.id = ? AND t.business_id = ? AND t.deleted_at IS NULL`
      )
