...
/** Per-store row counts — use to verify tenant isolation (products must not overlap by id across stores). */
router.get('/tenant-audit', async (req, res) => {
  try {
    const businesses = await db
      .prepare(
        `
      SELECT id, name, business_code, business_type FROM businesses ORDER BY name
    `
      )
      .all();
    ...
    for (const b of businesses) {
      ...
      const customers = Number(
        (await db
          .prepare(
            `SELECT COUNT(*) as c FROM customers WHERE business_id = ? AND deleted_at IS NULL`
          )
          .get(b.id))?.c ?? 0
      );
      const sales = Number(
        (await db
          .prepare(`SELECT COUNT(*) as c FROM sales WHERE business_id = ? AND deleted_at IS NULL`)
          .get(b.id))?.c ?? 0
      );
      stats.push({
        ...b,
        business_type: normalizeBusinessType(b.business_type),
        products,
        customers,
        sales,
      });
    ...
    const orphanProducts = Number(
      (await db
        .prepare(
          `SELECT COUNT(*) as c FROM products WHERE business_id IS NULL AND deleted_at IS NULL`
        )
        .get())?.c ?? 0
    );
    ...
    res.json({
      ...
      store_count: businesses.length,
    });
  } catch (e) {
    console.error('Tenant audit error:', e);
    res.status(500).json({ error: 'Failed to run tenant audit.' });
  }
});
...
router.patch('/businesses/:id', async (req, res) => {
  try {
    const { name, business_code, business_type, subscription_status, subscription_expires_at, notes } = req.body;
    ...
    if (business_code !== undefined) {
      const code = String(business_code).trim().toUpperCase();
      if (!code) {
        return res.status(400).json({ error: 'Store code cannot be empty.' });
      }
      const dup = await db
        .prepare(
          `SELECT id FROM businesses WHERE upper(trim(business_code)) = ? AND id != ?`
        )
        .get(code, req.params.id);
      if (dup) {
        return res.status(400).json({ error: 'Store code already in use by another business.' });
      }
...
router.post('/businesses/:id/notify-staff', async (req, res) => {
  try {
    ...
    const roles =
      target_role && ['admin', 'manager', 'cashier'].includes(target_role)
        ? [target_role]
        : ['admin', 'manager'];
    ...
    if (!created.length) {
      return res.status(500).json({ error: 'Failed to deliver notifications to store staff.' });
    }

    res.json({
      message: `Notification sent to store ${roles.join(' & ')} (${created.length} queued).`,
  ...
  try {
    ...
    const staff = await db
      .prepare(
        ...
        ORDER BY CASE role WHEN 'admin' THEN 1 WHEN 'manager' THEN 2 ELSE 3 END, lower(name)
      `
      )
      .all(req.params.id);
