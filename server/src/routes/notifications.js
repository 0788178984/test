...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    const staffParams = [req.user.business_id, req.user.id, req.user.role];
    ...
    `;
    const params = [req.user.id, req.user.role, ...staffParams];
...
const createNotification = async (notificationData) => {
  try {
    const id = notificationData.id || newId('notif');
...
const NOTIFICATION_EVENTS = {
  ...
  EXPENSE_RECORDED: {
    ...
    template: (meta) => ({
      ...
      message:
        meta.expense_type === 'stock_usage'
          ? `${meta.actor_name} recorded stock usage: ${meta.title} — UGX ${Number(meta.amount || 0).toLocaleString()} (inventory only).`
          : `${meta.actor_name} recorded cash expense: ${meta.title} — UGX ${Number(meta.amount || 0).toLocaleString()}.`,
    }),
  },
  ...
  EXPENSE_UPDATED: {
    ...
    template: (meta) => ({
      ...
      message: `${meta.actor_name} updated expense "${meta.title}" — UGX ${Number(meta.amount || 0).toLocaleString()}${meta.changes ? `. Changed: ${meta.changes}` : ''}.`,
    }),
  },
  ...
  EXPENSE_DELETED: {
    ...
    template: (meta) => ({
      ...
      message: `${meta.actor_name} removed expense "${meta.title}" — UGX ${Number(meta.amount || 0).toLocaleString()}.`,
    }),
  },

  CASH_FLOAT_OPENED: {
    type: 'cash_float_opened',
    severity: 'info',
    target_role: 'admin',
    channels: ['in_app'],
    template: (meta) => ({
      title: 'Cash Float Opened',
      message: `${meta.actor_name} opened float for ${meta.cashier_name}: cash UGX ${Number(meta.opening_cash || 0).toLocaleString()}, MoMo float UGX ${Number(meta.opening_float || 0).toLocaleString()}.`,
    }),
  },

  CASH_FLOAT_CLOSED: {
    type: 'cash_float_closed',
    severity: 'info',
    target_role: 'admin',
    channels: ['in_app'],
    template: (meta) => ({
      title: 'Cash Float Closed',
      message: `${meta.actor_name} closed ${meta.cashier_name}'s float. Cash variance UGX ${Number(meta.cash_variance || 0).toLocaleString()}, MoMo variance UGX ${Number(meta.float_variance || 0).toLocaleString()}.`,
    }),
  },

  CASH_TRANSACTION_RECORDED: {
    type: 'cash_transaction_recorded',
    severity: 'info',
    target_role: 'admin',
    channels: ['in_app'],
    template: (meta) => ({
      title: 'Cash / MoMo Transaction',
      message: `${meta.actor_name} recorded ${meta.transaction_type} of UGX ${Number(meta.amount || 0).toLocaleString()} (${meta.network}) on ${meta.cashier_name}'s float.`,
    }),
  },
  ...
  CASH_TRANSACTION_VOIDED: {
    ...
    channels: ['in_app'],
    template: (meta) => ({
      title: 'Cash Transaction Voided',
      message: `${meta.actor_name} voided ${meta.transaction_type} of UGX ${Number(meta.amount || 0).toLocaleString()} (${meta.network}) on ${meta.cashier_name}'s float.`,
    }),
  },
};
...
/** Notify both admin and manager (receipt, cash, expense, and other money changes). */
const dispatchToSupervisors = (eventType, meta = {}, options = {}) => {
  return SUPERVISOR_ROLES.map((target_role) =>
    dispatch(eventType, meta, { ...options, target_role })
  );
};
...
module.exports.dispatchToSupervisors = dispatchToSupervisors;
