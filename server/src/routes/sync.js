...
const { restrictToBusinessStaff, assertRecordTenant } = require('../middleware/tenantContext');
...
async function assertNoCrossTenantId(table, recordId, businessId) {
  if (!TABLES_WITH_BUSINESS_ID.has(table)) return { ok: true };
  const row = await db.prepare(`SELECT id, business_id FROM ${table} WHERE id = ?`).get(recordId);
  if (row?.business_id && row.business_id !== businessId) {
    return { ok: false, error: 'Record ID already belongs to another store.' };
  }
  return { ok: true };
}
    ...
    for (const record of records) {
      try {
        if (TABLES_WITH_BUSINESS_ID.has(table)) {
          const tenantCheck = assertRecordTenant(record, req.user.business_id, 'Sync record');
          if (!tenantCheck.ok) {
            conflicts.push({ id: record.id, type: 'tenant_violation', error: tenantCheck.error });
            continue;
          }
          if (!record.business_id) {
            record.business_id = req.user.business_id;
          }
        }
        ...
        if (!existing) {
          const idCheck = await assertNoCrossTenantId(table, record.id, req.user.business_id);
          if (!idCheck.ok) {
            conflicts.push({ id: record.id, type: 'tenant_violation', error: idCheck.error });
            continue;
          }
...
router.post('/resolve-conflict', authorize('admin'), async (req, res) => {
  try {
    ...
    if (TABLES_WITH_BUSINESS_ID.has(table)) {
      const tenantCheck = assertRecordTenant(data, req.user.business_id, 'Resolution data');
      if (!tenantCheck.ok) {
    ...
    // Get current record (scoped to this store when applicable)
    let current;
    if (TABLES_WITH_BUSINESS_ID.has(table)) {
      current = await db
        .prepare(`SELECT * FROM ${table} WHERE id = ? AND deleted_at IS NULL AND business_id = ?`)
        .get(record_id, req.user.business_id);
      ...
      current = await db.prepare(`SELECT * FROM ${table} WHERE id = ? AND deleted_at IS NULL`).get(record_id);
    }
