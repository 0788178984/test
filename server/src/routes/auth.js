...
async function storeCount() {
  return Number((await db.prepare(`SELECT COUNT(*) as c FROM businesses`).get()).c || 0);
}

/** Resolve store code for login. Auto-fills only when exactly one store exists. */
async function resolveBusinessCodeForLogin(raw, res) {
  let code = raw !== undefined && raw !== null ? String(raw).trim().toUpperCase() : '';
  if (code) return code;

  const count = await storeCount();
  if (count === 1) {
    const row = await db.prepare(`SELECT business_code FROM businesses ORDER BY created_at LIMIT 1`).get();
    return String(row?.business_code || '').trim().toUpperCase();
  }

  if (count > 1) {
    res.status(400).json({
      error:
        'Store code is required when more than one store exists. Enter the code for the store you want (shown in the sidebar or Developer Console).',
    });
    return null;
  }
...
router.post('/login', async (req, res) => {
  try {
    ...
    const code = await resolveBusinessCodeForLogin(business_code, res);
    if (code === null) return;
    if (!code) {
      return res.status(400).json({ error: 'No store exists in this database yet.' });
    }
      ...
      if (ok) {
        ...
        return res.json({
          ...
          user: {
            ...
            business_code: null,
            business_name: null,
            payment_methods: { cash: true, mtn_momo: false, airtel_money: false, pesapal: false },
          },
        });
      }
      return res.status(401).json({ error: 'Invalid credentials.' });
    ...
    res.json({
      ...
      user: {
        ...
        payment_methods: business
          ...
          : { cash: true, mtn_momo: false, airtel_money: false, pesapal: false },
      },
    });
  } catch (error) {
