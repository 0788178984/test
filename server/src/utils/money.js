...
/** Optional wholesale shelf price — must be at or above cost if provided. */
function assertWholesaleNotBelowCost(buyingPrice, wholesalePrice) {
  if (wholesalePrice === undefined || wholesalePrice === null || String(wholesalePrice).trim() === '') {
    return { ok: true, wholesale: null };
  }
  const buy = roundUgx(buyingPrice);
  const wholesale = roundUgx(wholesalePrice);
  if (wholesale < buy) {
    return {
      ok: false,
      error: `Wholesale price (UGX ${wholesale.toLocaleString()}) cannot be lower than buying price (UGX ${buy.toLocaleString()}).`,
    };
  }
  return { ok: true, wholesale };
}

module.exports = {
  roundUgx,
  computeSaleTotals,
  assertSellingNotBelowCost,
  assertWholesaleNotBelowCost,
  calcWholesaleUnitPrice,
};
