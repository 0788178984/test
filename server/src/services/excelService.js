...
const { netProfitFromSummary, fetchExpensesSummary, profitMetricLabel, formatUgxReportValue } = require('../utils/reportProfit');
...
class ExcelService {
  ...
  async generateDailyReport(date, options = {}) {
    try {
      ...
      summarySheet.addRows([
        ...
        { metric: 'Revenue', value: formatUgxReportValue(data.summary.revenue || 0) },
        { metric: profitMetricLabel(data.summary.gross_profit ?? data.summary.profit, 'Gross Profit (after COGS)', 'Gross Loss (after COGS)'), value: formatUgxReportValue(data.summary.gross_profit ?? data.summary.profit ?? 0) },
        { metric: 'Cash Paid Out', value: formatUgxReportValue(data.summary.cash_expenses_total ?? 0) },
        { metric: profitMetricLabel(data.summary.profit, 'Net Profit (after COGS & cash out)', 'Net Loss (after COGS & cash out)'), value: formatUgxReportValue(data.summary.profit || 0, { showProfitTag: Number(data.summary.profit) > 0 }) },
        { metric: 'Stock Used from Shop', value: formatUgxReportValue(data.summary.stock_expenses_total ?? 0) },
        { metric: profitMetricLabel(data.summary.net_cash ?? (Number(data.summary.revenue || 0) - Number(data.summary.cash_expenses_total || 0)), 'Net Till Cash (revenue − cash out)', 'Till Deficit (revenue − cash out)'), value: formatUgxReportValue(data.summary.net_cash ?? (Number(data.summary.revenue || 0) - Number(data.summary.cash_expenses_total || 0)), { showProfitTag: Number(data.summary.net_cash ?? (Number(data.summary.revenue || 0) - Number(data.summary.cash_expenses_total || 0))) > 0 }) },
        { metric: 'Average Sale', value: formatUgxReportValue(Math.round((data.summary.revenue || 0) / (data.summary.sales_count || 1))) }
      ]);
  ...
  async generateProfitReport(from, to, options = {}) {
    try {
      ...
      const netProfit = profitSummary.profit;
      const avgMargin = totalRevenue > 0 ? (netProfit / totalRevenue * 100).toFixed(2) : '0.00';

      worksheet.addRow({ sale_number: 'Total Revenue', total_amount: totalRevenue });
      worksheet.addRow({ sale_number: 'Cost of Goods', total_amount: totalCost });
      worksheet.addRow({ sale_number: profitMetricLabel(grossProfit, 'Gross Profit (after COGS)', 'Gross Loss (after COGS)'), total_amount: grossProfit });
      worksheet.addRow({ sale_number: 'Cash Paid Out', total_amount: profitSummary.cash_expenses_total });
      worksheet.addRow({ sale_number: profitMetricLabel(netProfit, 'Net Profit (after COGS & cash out)', 'Net Loss (after COGS & cash out)'), total_amount: netProfit });
      worksheet.addRow({ sale_number: profitMetricLabel(totalRevenue - profitSummary.cash_expenses_total, 'Net Till Cash (revenue − cash out)', 'Till Deficit (revenue − cash out)'), total_amount: totalRevenue - profitSummary.cash_expenses_total });
      worksheet.addRow({ sale_number: profitMetricLabel(Number(avgMargin), 'Net Profit Margin %', 'Net Loss Margin %'), total_amount: avgMargin });

      // Style summary rows
      const startRow = worksheet.rowCount - 7;
