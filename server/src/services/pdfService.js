...
const { netProfitFromSummary, fetchExpensesSummary, profitMetricLabel, formatUgxReportValue, isReportLoss } = require('../utils/reportProfit');
...
class PDFService {
  ...
  async generateProfitReport(from, to, options = {}) {
    try {
      ...
      const expensesSummary = await fetchExpensesSummary(options.business_id, { from, to });
      const profitSummary = netProfitFromSummary(grossProfit, expensesSummary);
      const data = { sales, profitSummary };
  ...
  addDailySummary(doc, data) {
    ...
    const netCash = Number(data.summary.net_cash ?? (Number(data.summary.revenue || 0) - Number(data.summary.cash_expenses_total || 0)));

    const summary = [
      { label: 'Total Sales', value: data.summary.sales_count || 0 },
      { label: 'Revenue', value: formatUgxReportValue(data.summary.revenue || 0) },
      { label: profitMetricLabel(data.summary.gross_profit ?? data.summary.profit, 'Gross Profit (after COGS)', 'Gross Loss (after COGS)'), value: formatUgxReportValue(data.summary.gross_profit ?? data.summary.profit ?? 0) },
      { label: 'Cash Paid Out', value: formatUgxReportValue(data.summary.cash_expenses_total ?? 0) },
      { label: profitMetricLabel(data.summary.profit, 'Net Profit (after COGS & cash out)', 'Net Loss (after COGS & cash out)'), value: formatUgxReportValue(data.summary.profit || 0), isLoss: isReportLoss(data.summary.profit), isProfit: !isReportLoss(data.summary.profit) && Number(data.summary.profit) > 0 },
      { label: 'Stock Used from Shop', value: formatUgxReportValue(data.summary.stock_expenses_total ?? 0) },
      { label: profitMetricLabel(netCash, 'Net Till Cash (revenue − cash out)', 'Till Deficit (revenue − cash out)'), value: formatUgxReportValue(netCash, { showProfitTag: true }), isLoss: isReportLoss(netCash), isProfit: !isReportLoss(netCash) && netCash > 0 },
      { label: 'Average Sale', value: formatUgxReportValue(Math.round((data.summary.revenue || 0) / (data.summary.sales_count || 1))) }
    ];

    summary.forEach((item) => {
      const y = doc.y;
      doc.fontSize(12).font('Helvetica-Bold').text(`${item.label}:`, 50, y);
      if (item.isLoss) doc.fillColor('#B91C1C');
      else if (item.isProfit) doc.fillColor('#15803D');
      doc.fontSize(12).font(item.isLoss || item.isProfit ? 'Helvetica-Bold' : 'Helvetica').text(item.value, 200, y);
      if (item.isLoss || item.isProfit) doc.fillColor('#000000');
      doc.moveDown();
    });
  ...
  addProfitSummary(doc, data) {
    ...
    const grossProfit = data.profitSummary?.gross_profit ?? totalRevenue - totalCost;
    const cashOut = data.profitSummary?.cash_expenses_total ?? 0;
    const netProfit = data.profitSummary?.profit ?? grossProfit - cashOut;
    const netCash = totalRevenue - cashOut;
    const profitMargin = totalRevenue > 0 ? (netProfit / totalRevenue) * 100 : 0;

    doc.fontSize(16).font('Helvetica-Bold').text('Profit Summary').moveDown();

    const summary = [
      { label: 'Total Revenue', value: formatUgxReportValue(totalRevenue) },
      { label: 'Cost of Goods', value: formatUgxReportValue(totalCost) },
      { label: profitMetricLabel(grossProfit, 'Gross Profit (after COGS)', 'Gross Loss (after COGS)'), value: formatUgxReportValue(grossProfit) },
      { label: 'Cash Paid Out', value: formatUgxReportValue(cashOut) },
      { label: profitMetricLabel(netProfit, 'Net Profit (after COGS & cash out)', 'Net Loss (after COGS & cash out)'), value: formatUgxReportValue(netProfit, { showProfitTag: netProfit > 0 }), isLoss: isReportLoss(netProfit), isProfit: netProfit > 0 },
      { label: profitMetricLabel(netCash, 'Net Till Cash (revenue − cash out)', 'Till Deficit (revenue − cash out)'), value: formatUgxReportValue(netCash, { showProfitTag: netCash > 0 }), isLoss: isReportLoss(netCash), isProfit: netCash > 0 },
      { label: profitMetricLabel(profitMargin, 'Net Profit Margin', 'Net Loss Margin'), value: `${profitMargin.toFixed(2)}%`, isLoss: isReportLoss(netProfit) }
    ];
  ...
  async getDailyData(date, options) {
    ...
    const expensesSummary = await fetchExpensesSummary(bid, { date });
