...
const TEMPLATE_COLUMNS = [
  ...
  { header: 'wholesale_price', key: 'wholesale_price', width: 14 },
...
class ProductImportService {
  async buildTemplateBuffer(businessType) {
    ...
    instructions.addRows([
      ...
      { field: 'selling_price', required: 'Yes', description: 'Retail shelf price in UGX. Must be ≥ buying price.' },
      {
        field: 'wholesale_price',
        required: 'No',
        description: 'Bulk/wholesale price in UGX. Optional; must be ≥ buying price if set.',
      },
  ...
  async importProducts(rows, { businessId, businessType, userId }) {
    ...
    for (const row of rows) {
      try {
        ...
        const wholesaleCheck = assertWholesaleNotBelowCost(priceCheck.buy, row.wholesale_price);
        if (!wholesaleCheck.ok) throw new Error(wholesaleCheck.error);
