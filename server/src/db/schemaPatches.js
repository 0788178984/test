...
async function applyCreditSalesPatch(pool) {
  ...
  try {
    ...
    await pool.query(`
      ...
      CHECK (payment_method IN ('cash', 'mtn_momo', 'airtel_money', 'mixed', 'credit', 'pesapal'))
    `);
  } catch (e) {
...
async function applyTenantIsolationPatch(pool) {
  if (!pool) return;

  const tenantTables = ['products', 'suppliers', 'customers', 'sales'];
  ...
  for (const table of tenantTables) {
    try {
      ...
      console.warn(`[schema] orphan ${table} cleanup:`, e.message);
    }

    try {
      await pool.query(`ALTER TABLE ${table} ALTER COLUMN business_id SET NOT NULL`);
    } catch (e) {
      if (e.code !== '23502') {
        console.warn(`[schema] ${table}.business_id NOT NULL:`, e.message);
      }
    }
  }
  ...
  const indexSteps = [
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_products_tenant_barcode
      ON products (business_id, lower(trim(barcode)))
    ...
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_products_tenant_sku
      ON products (business_id, lower(trim(sku)))
      WHERE sku IS NOT NULL AND trim(sku) <> '' AND deleted_at IS NULL`,
    `CREATE INDEX IF NOT EXISTS idx_suppliers_business ON suppliers (business_id)`,
    `CREATE INDEX IF NOT EXISTS idx_customers_business ON customers (business_id)`,
  ];

  for (const sql of indexSteps) {
    try {
      await pool.query(sql);
    } catch (e) {
      console.warn('[schema] tenant index:', e.message);
    }
  }
}

async function applyWholesalePricePatch(pool) {
  if (!pool) return;
  try {
    await pool.query(`ALTER TABLE products ADD COLUMN IF NOT EXISTS wholesale_price DOUBLE PRECISION`);
  } catch (e) {
    if (e.code !== '42701') console.warn('[schema] wholesale_price column:', e.message);
  }
}

/** Daily expenses: cash_out (reduces till cash) vs stock_usage (reduces inventory only). */
async function applyExpensesEnhancementPatch(pool) {
  if (!pool) return;

  const columnSteps = [
    `ALTER TABLE expenses ADD COLUMN IF NOT EXISTS expense_type TEXT DEFAULT 'cash_out'`,
    `ALTER TABLE expenses ADD COLUMN IF NOT EXISTS product_id TEXT REFERENCES products(id)`,
    `ALTER TABLE expenses ADD COLUMN IF NOT EXISTS quantity DOUBLE PRECISION`,
    `ALTER TABLE expenses ADD COLUMN IF NOT EXISTS stock_adjustment_id TEXT`,
  ];
  ...
  for (const sql of columnSteps) {
    try {
      ...
      if (e.code !== '42701') console.warn('[schema] expenses enhancement column:', e.message);
    }
  }

  try {
    await pool.query(`
      UPDATE expenses SET expense_type = 'cash_out'
      WHERE expense_type IS NULL OR trim(expense_type) = ''
    `);
  } catch (e) {
    console.warn('[schema] expenses expense_type backfill:', e.message);
  }

  try {
    await pool.query(`ALTER TABLE stock_adjustments DROP CONSTRAINT IF EXISTS stock_adjustments_adjustment_type_check`);
    await pool.query(`
      ALTER TABLE stock_adjustments
      ADD CONSTRAINT stock_adjustments_adjustment_type_check
      CHECK (adjustment_type IN ('restock', 'damage', 'return', 'correction', 'opening', 'usage'))
    `);
  } catch (e) {
    if (e.code !== '42710') console.warn('[schema] stock_adjustments usage type:', e.message);
  }
}
...
async function applyAllSchemaPatches(pool) {
  ...
  await applyTenantIsolationPatch(pool);
  await applyWholesalePricePatch(pool);
  await applyExpensesEnhancementPatch(pool);
}

module.exports = {
  applyBusinessTypePatch,
  applyCreditSalesPatch,
  applyTenantIsolationPatch,
  applyWholesalePricePatch,
  applyExpensesEnhancementPatch,
  applyAllSchemaPatches,
};
