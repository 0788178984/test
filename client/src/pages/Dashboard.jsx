...
import { formatCurrency, formatDate, getStoreToday, addStoreDays, getPurchaseDayLabel } from '../api/client';
...
const Dashboard = () => {
  ...
  const isSupervisor = hasRole('admin', 'manager');
  const [salesLogRange, setSalesLogRange] = useState(isSupervisor ? 'week' : 'today');
  const [stats, setStats] = useState({
    ...
    totalCustomers: 0,
    recentSales: [],
    salesLogTitle: "Today's Sales",
    salesLogShowDay: false,
  });

  useEffect(() => {
    fetchDashboardData();
  }, [user?.business_id, salesLogRange]);

  const getSalesLogWindow = (today) => {
    if (!isSupervisor || salesLogRange === 'today') {
      return { from: today, to: today, title: "Today's Sales", showDay: false, limit: 5 };
    }
    if (salesLogRange === 'yesterday') {
      const yesterday = addStoreDays(today, -1);
      return { from: yesterday, to: yesterday, title: "Yesterday's Sales", showDay: false, limit: 50 };
    }
    return {
      from: addStoreDays(today, -6),
      to: today,
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      setStoreDate(today);
      const salesLog = getSalesLogWindow(today);
      ...
      const recentSalesResponse = await salesAPI.getAll({
        ...
        to: salesLog.to,
        status: 'completed',
        limit: salesLog.limit,
      });

      let todayCashOut = 0;
      let todayStockUsed = 0;
      if (hasRole('admin', 'manager', 'cashier')) {
        try {
          const expRes = await expensesAPI.getTodaySummary({ date: today });
          todayCashOut = expRes.data.cash_total || 0;
          todayStockUsed = expRes.data.stock_total || 0;
        } catch {
          todayCashOut = 0;
          todayStockUsed = 0;
        }
      ...
      setStats({
        ...
        salesLogTitle: salesLog.title,
        salesLogShowDay: salesLog.showDay,
      });
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      <Card>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-lg font-semibold text-gray-900">{stats.salesLogTitle || "Today's Sales"}</h2>
            {isSupervisor && stats.salesLogShowDay ? (
              <p className="text-xs text-gray-500">Grouped by store day — use filters to view a single day in Reports</p>
            ) : null}
          </div>
              ...
              <div className="flex gap-1 rounded-lg border border-gray-200 bg-gray-50 p-1">
                {[
                  ...
                  <button
                    key={option.id}
                    type="button"
                    onClick={() => setSalesLogRange(option.id)}
                    className={`rounded-md px-2.5 py-1 text-xs font-medium transition-colors ${
                      salesLogRange === option.id
        ...
        {stats.recentSales.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            <p>{isSupervisor && salesLogRange !== 'today' ? 'No sales for this period' : 'No sales today yet'}</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="table">
              <thead>
                <tr>
                  {stats.salesLogShowDay ? <th>Day</th> : null}
                  <th>Receipt #</th>
                  <th>Customer</th>
                  <th>Cashier</th>
                  <th>Amount</th>
              ...
              <tbody>
                {stats.recentSales.map((sale) => (
                  <tr key={sale.id}>
                    {stats.salesLogShowDay ? (
                      <td className="whitespace-nowrap text-sm text-gray-600">
                        {getPurchaseDayLabel(sale.created_at)}
                      </td>
                    ) : null}
