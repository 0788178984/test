...
const Products = () => {
  ...
  useEffect(() => {
    setProducts([]);
    fetchProducts();
  }, [searchQuery, selectedCategory, page, user?.business_id]);
  ...
  const handleEditProduct = (product) => {
    ...
    setFormData({
      ...
      stock_to_add: '',
      set_stock: product.current_stock ?? 0,
  ...
  const buildPayload = () => {
    ...
    const wholesaleRaw = formData.wholesale_price;
    if (wholesaleRaw !== undefined && wholesaleRaw !== null && String(wholesaleRaw).trim() !== '') {
      base.wholesale_price = parseFloat(wholesaleRaw);
    ...
    if (editingProduct?.id) {
      ...
      const originalStock = Number(editingProduct.current_stock ?? 0);
      const setStock = parseFloat(formData.set_stock);
      if (Number.isFinite(setStock) && setStock >= 0 && setStock !== originalStock) {
        base.set_stock = setStock;
      }
      return base;
    }
    return {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      ...
      payload.wholesale_price < payload.buying_price
    ) {
      toast.error('Wholesale price cannot be lower than buying price');
      return;
    }
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <Input
              label="Wholesale price (UGX, optional)"
              ...
              onChange={handleInputChange}
              placeholder="Bulk price for wholesale POS mode"
            />
            
            {editingProduct?.id ? (
              <>
                <Input
                  label="Stock on hand (set count)"
                  name="set_stock"
                  type="number"
                  min="0"
                  step="any"
                  value={formData.set_stock ?? ''}
                  onChange={handleInputChange}
                  placeholder="Correct total stock count"
                />
                <Input
                  ...
                  onChange={handleInputChange}
                  placeholder="Adds on top of stock on hand"
                />
              </>
            ) : (
              <Input
