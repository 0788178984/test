...
export const paymentsAPI = {
  ...
  initiatePesapal: (data) => api.post('/api/payments/pesapal/initiate', data),
  pesapalStatus: (orderTrackingId) =>
    api.get('/api/payments/pesapal/status', { params: { order_tracking_id: orderTrackingId } }),
  pesapalWait: (orderTrackingId, timeoutMs = 45000) =>
    api.get('/api/payments/pesapal/wait', {
      params: { order_tracking_id: orderTrackingId, timeout_ms: timeoutMs },
...
export const formatCurrencyParts = (amount) => {
  ...
  return { minus, symbol: symbol.trim(), number, literal };
};
