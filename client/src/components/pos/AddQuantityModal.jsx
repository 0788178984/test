  ...
  const submit = () => {
    ...
    if (wholesaleMode) {
      if (presetWholesale != null) {
        onConfirm(n, { wholesaleMarkupPercent: 0, usesPresetWholesale: true });
        return;
      }
      ...
      >
        ...
        {wholesaleMode && (
          <div className="mb-4 rounded-lg border border-violet-200 bg-violet-50 p-3">
            {presetWholesale != null ? (
              <>
                <p className="text-sm font-medium text-violet-900">Wholesale price (preset)</p>
                <p className="mt-1 text-lg font-semibold text-violet-950">
                  {formatCurrency(presetWholesale)} per {label}
                </p>
                <p className="mt-1 text-xs text-violet-700">
                  Set on the product when stocking — no markup needed at sale time.
                </p>
              </>
            ) : (
              <>
                ...
                <p className="mt-1 text-xs text-violet-600">
                  Tip: set a fixed wholesale price on the product to skip markup here.
                </p>
              </>
            )}
          </div>
