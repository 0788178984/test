...
import { X, CreditCard, Smartphone, BookOpen } from 'lucide-react';
...
const PaymentModal = ({ totalAmount, customer, paymentMethods, canUseCredit, onPayment, onCancel }) => {
  ...
  const handlePayClick = async () => {
    ...
    if (paymentMethod === 'pesapal') {
      if (!phoneNumber || phoneNumber.replace(/\D/g, '').length < 9) {
        toast.error('Enter a valid customer phone number for Pesapal.');
        return;
      }
      ...
      try {
        ...
        const { data } = await paymentsAPI.initiatePesapal({
          amount,
          phone: phoneNumber,
          email: pesapalEmail.trim() || undefined,
        ...
        toast('Customer pays in Pesapal window — waiting for confirmation…', { icon: '⏳' });

        const completePesapal = (st) => {
          const ref = st.confirmation_code || trackingId;
          setReference(ref);
          onPayment({
            method: 'pesapal',
            amountPaid: amount,
        ...
        try {
          const { data: waited } = await paymentsAPI.pesapalWait(trackingId, 45000);
          if (waited.status_code === 1) {
            completePesapal(waited);
            return;
          }
          if (waited.status_code === 2 || waited.status_code === 3) {
            toast.error(waited.payment_status_description || 'Pesapal payment was not completed.');
            return;
          }
        } catch (_) {
          /* fall through to short backoff polling */
        }

        const backoffMs = [3000, 5000, 8000, 12000, 15000];
        for (const delay of backoffMs) {
          await sleep(delay);
          const { data: st } = await paymentsAPI.pesapalStatus(trackingId);
          if (st.status_code === 1) {
            completePesapal(st);
            return;
          }
          if (st.status_code === 2 || st.status_code === 3) {
            toast.error(st.payment_status_description || 'Pesapal payment was not completed.');
            return;
          }
        }
        toast.error('Payment timed out. Check Pesapal or try again.');
      } catch (error) {
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {methods.map((method) => {
            ...
            return (
              ...
              >
                {method.image ? (
                  <img src={method.image} alt={method.name} className={`mb-2 h-8 w-8 ${active ? '' : 'opacity-70'}`} />
                ) : (
        ...
        <div className="space-y-4">
          <div>
            <label className="form-label">
              {paymentMethod === 'pesapal' ? 'Customer phone (Pesapal)' : 'Payer phone (MoMo)'}
            </label>
            <input
              type="tel"
              value={phoneNumber}
          ...
          {paymentMethod === 'pesapal' && (
            <div>
              ...
              <p className="mt-1 text-xs text-gray-500">
                Opens Pesapal in a new tab — customer pays with card, MoMo, or other Pesapal methods.
              </p>
            </div>
          )}
      ...
      {isProcessing && paymentMethod !== 'cash' && paymentMethod !== 'credit' && (
        <div className="flex items-center gap-3 rounded-lg border border-yellow-200 bg-yellow-50 p-4">
          ...
          <p className="text-sm text-yellow-800">
            {paymentMethod === 'pesapal'
              ...
              : `Requesting ${getPaymentMethodName(paymentMethod)}…`}
          </p>
        </div>
      )}
