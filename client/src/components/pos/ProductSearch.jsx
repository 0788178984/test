...
const ProductSearch = ({
  ...
  const fetchBrowse = async (page = 1, append = false) => {
    ...
    try {
      ...
      setBrowsePage(pg.page || page);
      setBrowseTotal(pg.total || list.length);
      setBrowsePages(pg.pages || 1);
    } catch (error) {
  ...
  }, [searchQuery, selectedCategory, user?.business_id]);
      ...
      {!loading && (
        <div className="product-grid-pos max-h-[28rem] overflow-y-auto pr-1">
          {products.length === 0 ? (
            ...
            <>
            {browseTotal > products.length && (
              <p className="col-span-full mb-1 text-xs text-gray-500">
                Showing {products.length} of {browseTotal} products — scroll and load more to see the rest (A–Z).
              </p>
            )}
            ...
            {browsePage < browsePages && (
              <div className="col-span-full py-3 text-center">
                <button
                  ...
                  onClick={() => fetchBrowse(browsePage + 1, true)}
